...
const getPartial = (start, end, p_data) => {
  ...
  for (let i = 0; i < p_data.length; i++) {
    if (
      parseTime(p_data[i].Date) >= start &&
      parseTime(p_data[i].Date) <= end
    ) {
...
const AreaGraph = ({ data, currency, column, width, height }) => {
  ...
  const g_h = (height * 5) / 6 - margins * 2

  const s_w = width - margins * 2

  const s_h = height / 6 - margins

  const dates = data.map(d => parseTime(d.Date))
  let rangeStart = parseTime(areaStore.partial[0].Date)

  let rangeEnd = parseTime(areaStore.partial[areaStore.partial.length - 1].Date)
  ...
  const drawAreaGraph = (data, w, h) => {
    ...
    let max = d3.max(area_data, d => d.value)
    ...
    chart
      ...
      .on('mousemove', function () {
        try {
          ...
          let i = bisectDate(area_data, x0, 0)

          let d0 = area_data[i - 1]
    ...
    const callout = (g, value) => {
      ...
      path
        .attr(
          'd',
          `M${-tw / 2 - 10},5H-5l5,-5l5,5H${tw / 2 + 10}v${th + 10}h-${tw +
            20}z`
        )
  ...
  useEffect(
    () => {
      ...
      d3.select('.rectFillBar').attr(
        'width',
        xScale(rangeEnd) - xScale(rangeStart)
      )
    },
  ...
  useEffect(
    () => {
      ...
      let max = d3.max(c_data[0].values, d => d.value)
      let y = d3
        .scaleLinear()
        .range([s_h, 0])
        .domain([min, max])
  ...
  return (
    <svg width={width} height={height}>
      ...
      <g
        className='sliderBar'
        transform={`translate(${margins}, ${(height * 5) / 6})`}
      >
...
const AreaGraphContainer = props => {
  ...
  const sorted_data = preCorrection(data).sort((x, y) =>
    d3.ascending(parseTime(x.Date), parseTime(y.Date))
  )
  ...
  const areaReducer = (state, action) => {
    ...
    switch (action.type) {
      case 'RANGE_1_MONTH':
        ...
        return {
          partial: getPartial(parseTime(start), parseTime(end), sorted_data)
        }
      case 'RANGE_6_MONTH':
        end = sorted_data[sorted_data.length - 1].Date
        start =
    ...
    <AreaContext.Provider value={{ areaStore, areaDispatch }}>
      <div>
        <div className='columns'>
          ...
          >
            <span
              style={{ color: '#de0730', fontWeight: '600', fontSize: '15pt' }}
            >
              ●&nbsp;
            </span>
