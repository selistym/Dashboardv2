...
const SecuritiesSection = ({
  ...
  const availableForArea = sc => {
    ...
    if (
      sc == null ||
      sc.calculatedCircular == null ||
      sc.calculatedCircular[0].Year != store.securityFilterYear
    ) { return false }

    if (store.securityFilterArea.length == 0) return true

    if (
      store.securityFilterArea.indexOf('Balance') != -1 &&
      sc.calculatedCircular[0].Balance >= 75
    ) {
      canBeShow[0] = true
    }
    if (
      store.securityFilterArea.indexOf('Dividend') != -1 &&
      sc.calculatedCircular[0].Dividend >= 75
    ) {
      canBeShow[1] = true
    }
    if (
      store.securityFilterArea.indexOf('Growth') != -1 &&
      sc.calculatedCircular[0].Growth >= 75
    ) {
      canBeShow[2] = true
    }
    if (
      store.securityFilterArea.indexOf('Value') != -1 &&
      sc.calculatedCircular[0].Value >= 75
    ) {
      canBeShow[3] = true
    }
