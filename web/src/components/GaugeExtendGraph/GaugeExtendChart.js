...
class GaugeChart extends Component {
  ...
  drawChart() {
    ...
    let range = Math.abs(dataRange.max - dataRange.min),
      ...
      preStep = preStep1 < -100 ? -100 : preStep1,
    ...
    if (dataRange.direction == 1)
      linearColor = d3
        .scaleLinear()
        // .range(['#e2062a', '#f33b59', '#e2e6ea', '#b8f783', '#66ad2b'])
        // .domain([0, range/4, range/2, range*3/4, range]);
    ...
    let arc = d3
      ...
      .outerRadius(radius)
      .startAngle((d, i) => scale(i - 100))
      .endAngle((d, i) => scale(i + 1 - 100));
    ...
    if (kind == 0)
      ...
      d3.select(this.el)
        ...
        .enter()
        .append('path')
        .attr('stroke', (d, i) => {console.log(step1); return i + 1 - 100 <= data[0].value ? linearColor(step1) : '#e4e7ec'})
        .attr('fill', (d, i) => i + 1 - 100 <= data[0].value ? linearColor(step1) : '#e4e7ec')
        .attr('d', arc);
    //draw needle
