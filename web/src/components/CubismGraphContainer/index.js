...
const CubismGraph = ({ data, period, width }) => {
  ...
  const setShowDateInfo = period => {
    switch (period) {
      case 'All':
        var max_date = data[0].globalQuotes[0].date

        var min_date = data[0].globalQuotes[0].date
        for (let i = 0; i < data.length; i++) {
          if (
            new Date(max_date) <
            new Date(data[i].globalQuotes[data[i].globalQuotes.length - 1].date)
          ) {
            max_date =
              data[i].globalQuotes[data[i].globalQuotes.length - 1].date
          }
      ...
      case '1 M':
        ...
        fromDate = new Date(new Date(toDate) - dateLen * 24 * 3600 * 1000)
          .toISOString()
          .slice(0, 10)
        break
      case '6 M':
  ...
  const getConvertData = input_data => {
    ...
    let prv_data = { date: fromDate, close: 0, is_null: true }
  ...
  const data2 = data.map(d => {
    return Object.assign({}, d, {
      globalQuotes: d.globalQuotes.sort((x, y) => d3.ascending(x.date, y.date))
    })
  })
  ...
  useEffect(() => {
    ...
    var elemRect = graphRef.current.getBoundingClientRect()
    ...
    d3.selectAll('.title').html(
      (d, i) => `<a href="/security/${data[i].id}">${data[i].name}</a>`
    )
    ...
    function stock (datum) {
      ...
      return c.metric(function (start, stop, step, callback) {
        ...
        let initValue =
            datum.globalQuotes[0].close == 0
              ? getNearest(datum.globalQuotes)
              : datum.globalQuotes[0].close
  ...
  return (
    <div
      ref={graphRef}
      style={{ height: '40px', marginLeft: '20px', justifyContent: 'center' }}
    >
      <style jsx global>{`
        .group {
          margin-bottom: 1em;
