...
const StockGraph = ({ data, width, height }) => {
  ...
  const drawGraph = (w, h, param) => {
    ...
    let min = d3.min(param.map(d => d.ConsolidatedNetIncome))
    ...
    stock
      ...
      .attr('d', d => {
        ...
        let h = Math.abs(y(d.ConsolidatedNetIncome) - y(0))
    ...
    stock
      ...
      .attr('d', function (d) {
        ...
        let h = Math.abs(y((d.ConsolidatedNetIncome * ratio) / 100) - y(0))
    ...
    stock
      ...
      .text(d =>
        d.DividendPayoutRatio > 100
          ? 100 + '%'
          : Math.floor(d.DividendPayoutRatio) + '%'
      )
    stock
  ...
  useEffect(() => drawGraph(width - margin, height, data), 
...
const StockGraphContainer = ({ data }) => {
  ...
  const preCorrection = params => {
    return params.map(d => {
      d.ConsolidatedNetIncome = d.ConsolidatedNetIncome
        ? d.ConsolidatedNetIncome
        : 0
      d.ConsolidatedNetIncomeEUR = d.ConsolidatedNetIncomeEUR
        ? d.ConsolidatedNetIncomeEUR
        : 0
  ...
  return (
    <>
      <div className='columns'>
        <div
          className='column'
          style={{ padding: 0, textAlign: 'center', fontSize: '18pt' }}
        >
      ...
      <div className='columns'>
        ...
        >
          <div ref={svgContainerRef}>
            {svgSize.width && (
              <StockGraph
                data={preCorrection(data).reverse()}
                width={svgSize.width}
                height={265}
              />
            )}
      ...
      <div
        className='columns'
        style={{ textAlign: 'center', fontSize: '11pt' }}
      >
